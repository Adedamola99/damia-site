...
export default function Sidebar() {
  ...
  return (
    <aside className="h-screen bg-background text-primary-text flex flex-col items-center py-6 shadow-lg transition-all duration-300 ease-in-out sidebar">
      <div className="mb-8">
        <Image src={Logo} alt="Next.js logo" width={100} height={38} priority />
      </div>

      <nav className="space-y-6 flex flex-col gap-3">
        <Link
          href="/"
          className="group relative text-2xl hover:text-primary transition-transform duration-300 transform hover:scale-110"
        >
          <AiFillHome
      ...
      <div className="mt-auto">
        <div className="text-sm text-gray-900 dark:text-white">
          <div
            onClick={toggleTheme}
            className={`relative inline-flex items-center h-6 w-11 cursor-pointer transition-colors ${
              theme === "dark" ? "bg-yellow-500" : "bg-gray-200"
            } rounded-full`}
          >
            <span
            ...
            />
            <FaMoon
              className={`absolute left-1 text-gray-400 transition-opacity ${
                theme === "dark" ? "opacity-100" : "opacity-0"
              }`}
            />
            <FaSun
              className={`absolute right-1 text-gray-400 transition-opacity ${
